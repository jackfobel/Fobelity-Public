...
public class HostAuthenticationStateProvider : AuthenticationStateProvider
{
  ...
  private async Task<ClaimsPrincipal> FetchUser()
  {
    ...
    var claims = user.Claims?.Select(c => new Claim(c.Type, c.Value)) ?? Enumerable.Empty<Claim>();

    // ✅ This MUST NOT be null or empty
    var identity = new ClaimsIdentity(
        claims,
        "Cookies", // 🔑 This MUST be set to any non-null string
        user.NameClaimType ?? ClaimTypes.Name,
        user.RoleClaimType ?? ClaimTypes.Role
    );
