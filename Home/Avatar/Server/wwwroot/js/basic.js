...
function connectToAvatarService(peerConnection) {
    ...
    .then(response => {
        if (response.ok) {
          ...
          //alert('You need to use the S0 Speech service instead of the Free service for this functionality.');
          document.getElementById('logging').innerHTML = 'You need to use the S0 Speech service account instead of the Free service for this functionality.';
          document.getElementById('logging').focus();
        }
    })
}
